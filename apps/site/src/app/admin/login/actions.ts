...
export async function signIn(_prev: ActionState, formData: FormData): Promise<ActionState> {
  ...
  try {
    ...
    if (verified.length > 0) redirect('/admin/login/verificacao');

    // Sem MFA cadastrado ainda: manda configurar antes de liberar o painel.
    redirect('/admin/configuracoes/seguranca?exigirMfa=1');
  } catch (error) {
...
export async function requestPasswordReset(
  ...
  try {
    ...
    await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: `${adminUrl}/auth/callback?proximo=/admin/redefinir-senha`,
    });
